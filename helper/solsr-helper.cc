...
{
  m_agentFactory.SetTypeId ("ns3::sally::SOlsrRoutingProtocol");
}
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      Ptr<sally::SOlsrRoutingProtocol> solsr = DynamicCast<sally::SOlsrRoutingProtocol> (proto);
      if (solsr)
        {
      ...
      if (list)
        {
          ...
          Ptr<sally::SOlsrRoutingProtocol> listSOlsr;
          for (uint32_t i = 0; i < list->GetNRoutingProtocols (); i++)
            {
              listProto = list->GetRoutingProtocol (i, priority);
              listSOlsr = DynamicCast<sally::SOlsrRoutingProtocol> (listProto);
              if (listSOlsr)
                {
